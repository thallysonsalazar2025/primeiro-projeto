...
4. o manifesto bruto preserva a base de uso e a licença/termos verificados, quando aplicáveis;
...
- `documentUrl`: URL concreta do PDF/CSV/ZIP selecionado após descoberta;
...
3. selecionar explicitamente o artefato desejado (por exemplo, prova de Análise de Sistemas e respectivo gabarito) e construir `OfficialDocumentCandidate` com `sourceUrl`, `documentUrl`, `documentType`, `usageBasis`, `license`, `termsUrl`, `retrievedAt` e `sha256`;
4. implementar download seguro reutilizando o gate HTTP/SSRF e limites já existentes;
5. armazenar o documento bruto e manifesto de proveniência fora das filas `questions`/`rankings`;
...
A descoberta não deve seguir qualquer link arbitrário da landing page. Ela deve restringir protocolo, host/redirects, tipo de conteúdo e escopo esperado, e precisa manter a relação auditável `sourceUrl -> documentUrl` no manifesto.
