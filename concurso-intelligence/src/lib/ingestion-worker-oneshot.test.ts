...
test('worker oneshot processa fila nova mesmo após falha na recuperação stale', async () => {
  const inbox = await mkdtemp(join(tmpdir(), 'ingestion-oneshot-recovery-'));

  try {
    const processingQuestionsDir = join(inbox, 'processing', 'questions');
    const rankingsDir = join(inbox, 'rankings');
    await mkdir(processingQuestionsDir, { recursive: true });
    await mkdir(rankingsDir, { recursive: true });

    const staleQuestion = join(processingQuestionsDir, '1-123-interrupted.json');
    await writeFile(staleQuestion, '{json-invalido', 'utf8');
    await utimes(staleQuestion, new Date(0), new Date(0));
    await writeFile(join(rankingsDir, 'invalid-ranking.json'), '{json-invalido', 'utf8');
    ...
    const result = spawnSync(
      ...
      ['--experimental-strip-types', 'scripts/ingestion-worker.ts'],
      {
        ...
        env: {
          ...
          INGESTION_INBOX_DIR: inbox,
          INGESTION_ONESHOT: 'true',
          INGESTION_STALE_CLAIM_SECONDS: '1',
        },
        encoding: 'utf8',
        timeout: 10_000,
      },
    ...
    const failedRankings = await readdir(join(inbox, 'failed', 'rankings'));
    assert.equal(failedQuestions.length, 1);
    assert.match(failedQuestions[0], /recovered-[0-9a-f]{16}\.json$/);
    assert.equal(failedRankings.length, 1);
    assert.match(failedRankings[0], /invalid-ranking\.json$/);
  } finally {
    await rm(inbox, { recursive: true, force: true });
